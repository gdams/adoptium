...
Nous figurons également dans tous les gestionnaires d'installation de Java courants tels que https://formulae.brew.sh/cask/temurin[Homebrew], https://sdkman.io/[SDKMAN],
https://github.com/microsoft/winget-cli[winget] et https://chocolatey.org/[Chocolatey], nous apparaissons dans les 
...
Les installateurs Linux sont disponibles sur packages.adoptium.net.
...
logiciel ou à diagnostiquer des problèmes, et en aidant à link:/docs/first-timer-support[résoudre les problèmes] qui ont été signalés.
...
Les tests AQAvit vérifient que la version fait preuve d'excellentes performances, la sécurité, la résilience et endurance, 
ainsi que la capacité de passer une large variété de tests de compatibilité avec des applications.
...
et nous verrons ensemble pour trouver une solution. Notre support est basé sur une lien:/support["communauté"] donc les problèmes sont résolus dans la mesure du possible. 
...
Nous avons une liste de variantes dans notre https://hub.docker.com/_/eclipse-temurin[dépôt officiel Docker] pour assurer la livraison de nouvelles images contenant Temurin.
...
Il s'agit du projet et du nom de marque des binaires que La Fondation Eclipse produit.
...
qui les produit qu'avant, mais il y a un link:/des membres[plus de grandes entreprises] apportant leur expertise au groupe de travail.

Pour assurer une transition en douceur, nous avons laissé l'ancien site actif. 
Cependant, les liens vers les dernières versions de l'ancien site vous amèneront désormais à celles de Temurin. 
...
Le passage à Adoptium fait que nous n'avons malheureusement pas pu continuer à distribuer les versions des builds d'Eclipse OpenJ9. 
...
Les accords, que nous avons depuis passés sous la Fondation Eclipse, signifient que nous ne pouvons plus inclure IcedTea-Web dans nos installeurs.
...
De manière générale, contacter l'équipe via Slack est le meilleur moyen de dialoguer avec nous.